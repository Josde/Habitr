...
class TimerWidget extends StatefulWidget {
  const TimerWidget({Key? key,
          ...
          required this.countsUp}) : super(key: key);
    ...
    timer = Timer.periodic(Duration(seconds: 1), (_) {
            setState(() {
              if (widget.countsUp) {
                ...
                _currentTimer--;
                _controller.animateTo(((_currentTimer / widget.lengthInSeconds) - 1).abs(), duration: Duration(milliseconds: 1050));
                if (_currentTimer <= 0) {
                  widget.onComplete();
                  timer.cancel();
                }
  ...
  Widget build(BuildContext context) {
    ...
    return Stack(children: [
                          ...
                          Padding(
                            ...
                            child: Center(child: Text(_timerString)),
                          ),
    ]);
  }
}